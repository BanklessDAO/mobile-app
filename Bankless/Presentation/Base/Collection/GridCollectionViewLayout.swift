...
protocol GridCollectionViewLayoutDelegate: AnyObject {
    ...
    func collectionView(
        _ collectionView: UICollectionView,
        heightForSupplementaryView kind: GridCollectionViewLayout.ElementKind, at section: Int
    ) -> CGFloat?
    func collectionView(
        _ collectionView: UICollectionView,
...
class GridCollectionViewLayout: UICollectionViewLayout {
    ...
    private func prepareCells(at section: Int) {
        ...
        let alignment = delegate?.collectionView(
            collectionView, alignmentForSection: section
        ) ?? cellAlignment
        
        while (itemsCount - itemIndex) > 0 {
            let columnWidth = (collectionViewWidth - CGFloat(numberOfColumns - 1) * columnSpacing)
                / CGFloat(numberOfColumns)
            let columnHeight = delegate?.collectionView(collectionView, heightForRow: row)
                ?? estimatedCellHeight
            ...
            (0..<numberOfColumns).forEach { column in
                ...
                let cellSpan = min(
                    ...
                    delegate?.collectionView(
                        collectionView, columnSpanForItemAt: index, indexPath: indexPath
                    ) ?? estimatedColumnSpan
                )
                
                let cellWidth = columnWidth * CGFloat(cellSpan)
                    + CGFloat(cellSpan - 1) * columnSpacing
                let cellHeight = min(
                    columnHeight,
                    delegate?.collectionView(
                        collectionView, heightForItemAt: index, indexPath: indexPath
                    ) ?? estimatedCellHeight
                )
    ...
    private func prepareSuplementaryView(kind: ElementKind, at section: Int) {
        guard let collectionView = collectionView else { return }
        guard let height = delegate?.collectionView(
            collectionView, heightForSupplementaryView: kind, at: section
        ) else { return }
        
        let indexPath = IndexPath(row: 0, section: section)
        let attributes = GridLayoutAttributes(
            forSupplementaryViewOfKind: kind.rawValue,
            with: indexPath
        )
        attributes.frame = CGRect(
            x: 0, y: collectionViewHeight,
            width: collectionViewWidth, height: height
        )
